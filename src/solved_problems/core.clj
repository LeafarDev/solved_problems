...
(divisors 104)
(Math/sqrt 1234)
(time (quantity-of-divisors 1234))

(defn quantity-of-divisors2 [n]
  (count (remove nil? (map (fn* [p1__266952#] (when (zero? (mod n p1__266952#)) p1__266952#)) (remove nil? (map (fn* [p1__262425#] (when (prime? p1__262425#) p1__262425#)) (range 1 (inc n))))))))
(time (quantity-of-divisors2 600))

(remove prime? (range 1 (inc 999999999)))
(loop [i 1 last-number 0]
  (if (prime? last-number)
    (recur (inc i) (+ last-number i))
    (if (<= last-number 500)
      (do (print (str "\n" last-number ":" (quantity-of-divisors last-number)))
          (recur (inc i) (+ last-number i)))
      last-number)))
